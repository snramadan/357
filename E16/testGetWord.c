...
static FILE* checkArgs(int argc, char *argv[])
{
   if (argc == 2)
   {
   ...
   fprintf(stderr, "Usage: getWord [-nomem] fileName\n");
   exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
   ...
   /* Print the last word returned with the EOF, if any */
   printWord(word, wordLength, hasPrintable);
   free(word);
