 ...
 *       free the data itself - that is should be done by htDestroy (see the
 *       description for the htDestroy function for details).
 */
 ...
 *    hash table. If the provided flag (destroyData) is true then all of the
 *    data stored in the hash table is freed too.
 *
 * Notes:
 *   1. If the destroyData flag is true AND the hash table's destroy function
 *      is not NULL then that destroy function is called BEFORE freeing the
 *      the data - the free function is always called by on the data by this
 *      function when destroyData is true.
